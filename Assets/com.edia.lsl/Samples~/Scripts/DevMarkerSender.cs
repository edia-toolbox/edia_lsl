...
public class DevMarkerSender : MonoBehaviour
{
	...
	while (true) {
			...
			MarkerOutletTest.SendMarker("Marker Sent EndOfFrame", MarkerOutlet.MomentForMarker.EndOfFrame); //gets sent at the end of the current frame
			MarkerOutletTest.SendMarker("Marker Sent Start Of Next Frame", MarkerOutlet.MomentForMarker.StartOfFrame); //gets sent at the start of the next frame; least jitter in my tests
